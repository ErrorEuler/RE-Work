...
class ScheduleManager {
    ...
    initialize(initialData = []) {
        ...
        console.log('📋 Schedule Manager initialized with', this.scheduleData.length, 'schedules');
    ...
    updateSchedule(scheduleId, updatedSchedule) {
        const index = this.scheduleData.findIndex(s => s.schedule_id == scheduleId);
        if (index !== -1) {
            this.scheduleData[index] = { ...this.scheduleData[index], ...updatedSchedule };
    ...
    deleteSchedule(scheduleId) {
        this.scheduleData = this.scheduleData.filter(s => s.schedule_id != scheduleId);
